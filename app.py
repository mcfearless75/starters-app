...
WHERE id NOT IN (
  ...
  GROUP BY supplier_name,supplier_contact,supplier_address,
           employee_name,address,ni_number,role_position,
           department,start_date,office_location,
           salary_details,probation_length,
...
def generate_pdf_bytes(fields):
    ...
    return pdfkit.from_string(html, False, configuration=cfg, options={"enable-local-file-access": None})
...
if page == "New Starter":
    ...
    with st.form("new_starter_form"):
        ...
        with c2:
            ...
            salary_details = st.text_area("Salary Details",height=80)  # ← increased from 60 to 80
        st.markdown('</div>', unsafe_allow_html=True)
        st.markdown("<hr>",unsafe_allow_html=True)
        ...
        # **Submit** button must live inside the form
    ...
    if submitted:
        ...
        try:
            ...
            st.download_button("⬇️ Download PDF",pdfb,
              file_name=f"new_starter_{employee_name.replace(' ','_')}.pdf",mime="application/pdf")
...
# ─── STARTER LIST VIEW & CRUD ───────────────────────────────────
    ...
    if df.empty:
        ...
        if edited is not None and st.button("💾 Save changes"):
            # deletes
            to_del = set(df_orig["id"]) - set(edited["id"])
            if to_del:
                c.executemany("DELETE FROM starters WHERE id=?",( (i,) for i in to_del ))
                st.write(f"🗑️ Deleted {len(to_del)} starter(s)")
            # updates
        ...
        # PDF regen & full-report omitted for brevity…
        # (same as before)
